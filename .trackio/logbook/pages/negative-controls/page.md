...
---
<!-- trackio-cell
{"type": "markdown", "id": "cell_sn_03", "created_at": "2026-07-23T11:23:18+00:00", "title": "Route-audit manifest tampering (not executed)"}
-->
**Committed but not executed.** Each route child contains a fail-closed audit harness that changes
one required SHA-256 value in memory and must reject the tampered manifest. The local runner died
before this control ran, so it is implementation evidence only and is not counted as a passing
negative control.
